...
def popup_message(text, tab):  # universally callable message-display
    ...
    message = text
    pop = Text(tab, height=7, width=42, font = 'Helvetica 16')
    pop.insert(END, text)
...
def main_login_student_operation(window, master):
    ...
    instruction = Label(window, text="Enter Student ID", font='Helvetica 16 bold', fg='purple4',bg='grey86')
...
def admin_duties(admin, tabStructure, master):  # admin operation
    ...
    text = Text(admin, height=18, width=50)
    DateField = Entry(admin, width=14, borderwidth=3)
    button1 = Button(admin, text="Who's In the Shop?", width=20, font='Helvetica 16 bold',command=whos_in_the_shop)
    button2 = Button(admin, text="Signout All", width=20,font='Helvetica 16 bold', command=logout_all_users)
    button3 = Button(admin, text="Blame", width=20,font='Helvetica 16 bold', command=tools_past_due)
    button4 = Button(admin, text="Edit Training", width=20, font='Helvetica 16 bold',command=edit_training)
    button5 = Button(admin, text="Add Key", width=20, font='Helvetica 16 bold',command=add_tool)
    ...
    text.grid(column=3, row=0, rowspan=10, columnspan=2)
    DateField.grid(column=1, row=7)
    prompt = Label(admin, text="Please enter the day", font='bold 12', bg='MediumPurple1')
    prompt.grid(column=1, row=6)
...
class KeyButton:
    ...
    def __init__(self, master, x, y, number, root, name, color):
        ...
        if svc.key_exists(number):
            self.button = Button(master, text=str(number) + '\n' + name, bg=color, width=12, height=2, command=Onclick)
...
def Checkout_tool(x, y, toolname):
    ...
    e = Entry(pop, font='Helvetica 16', width=25)
    ...
    date = Entry(pop, font='Helvetica 16', width=25)
    date.pack()
...
def Return_Tool(x, y, toolname):
    ...
    I.pack()
    E = Entry(pop, width=35, borderwidth=2, font='Helvetica 16')
    E.pack()
...
def tools_tab_functions(tools, root, tabStructure):
    ...
    toolName = Entry(tools, width=31, borderwidth=2, font='Arial 14')
    ...
    return_ID = Entry(tools, width=31, borderwidth=2, font='Arial 14')
